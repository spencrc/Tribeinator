...
const addRoleOption = (option: SlashCommandStringOption) =>
	option.setName('role').setDescription('The role name').setRequired(true);

const addSelfRole = async (
...
export default new SlashCommand({
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				.setName('add')
				.setDescription(
					'Allows you to give yourself a role from the server list'
				)
	...
	execute: async (interaction: ChatInputCommandInteraction): Promise<void> => {
		...
		try {
			...
			if (rowCount! === 0)
				throw new Error('role unexpectingly not in database'); // row count can never be null, it'll be 0 if no roles were selected
